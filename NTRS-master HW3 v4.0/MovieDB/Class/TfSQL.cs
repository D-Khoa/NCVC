...
namespace NTRS
{
    public class TfSQL
    {
        ...
        string conStringSerialDb = @"Server=192.168.193.2;Port=5432;User Id=pqm;Password=dbuser;Database=serinfdb; CommandTimeout=100; Timeout=100;";
        ...
        //SELECT Duplicate BARCODE
        public string sqlExecuteScalarStringSerial(string sql)
        {
            string response;
            try
            {
                connection = new NpgsqlConnection(conStringSerialDb);
                connection.Open();
                NpgsqlCommand command = new NpgsqlCommand(sql, connection);
                response = Convert.ToString(command.ExecuteScalar());
                connection.Close();
