...
namespace JigQuick
{
    public partial class frmNoJig99 : Form
    {
        ...
        // コンストラクタ
        public frmNoJig99(string applicationname)
        {
        ...
        private void txtChild_KeyDown(object sender, KeyEventArgs e)
        {
            // バーコード末尾のエンターキー以外は処理しない
            if (e.KeyCode != Keys.Enter) return;
            if (txtProcess.Text == "Seal Label-Form Pasting (D-6)" & txtChild.Text.Length > 0 & txtChild.Text.Length < 24)
            {
                MessageBox.Show("Product serialno must be 24 digits", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtChild.Clear();
                return;
            }
            // 空文字の場合は処理しない
