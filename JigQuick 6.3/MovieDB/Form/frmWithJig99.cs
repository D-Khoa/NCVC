...
namespace JigQuick
{
    public partial class frmWithJig99 : Form
    {
        ...
        // コンストラクタ
        public frmWithJig99(string applicationname)
        {
        ...
        private void txtChild_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (processMode == "Coil Assy (D-3)" || processMode == "Crust ID Reading (D-5)")
            {
                ...
                if (txtChild.Text.Length > 0 & txtChild.Text.Length < 24)
                {
                    MessageBox.Show("BB barcode must be 24 digits", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtChild.Clear();
                    return;
                }
            }
            else if (processMode == "Crust Tungsten Assy Welding (D-2)")
        ...
        private void txtChild2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            // ＰＡＲＥＮＴが埋まっていない場合は、ＰＡＲＥＮＴスキャン用にカーソルを移動
            else if (txtParent.Text == string.Empty && txtParent.Enabled == true)
            {
                txtParent.Focus();
            }
            // 親、子２人、全員テキストがそろった場合のみ、出力
        ...
        {
            if (txtParent.Text.Length > 17)
            {
                MessageBox.Show("CRUST barcode must be 17 digits", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtParent.Clear();
                return;
            }
            // バーコード末尾のエンターキー以外は処理しない
        ...
        private void checkAndOutput()
        {
            ...
            if (autoRegister == "ON")
            {
                if (processMode == "Coil Assy (D-3)")
                {
                    outputTbiInfoForCoilAssy();
                }
                else if (processMode == "Crust Tungsten Assy Welding (D-2)")
                {
                    outputTbiInfoForCrustTungWelding();
                }
                else if (processMode == "Crust ID Reading (D-5)")
                {
                    outputTbiInfoForCrustReading();
                }
            }
        }

        // サブプロシージャ：テキストファイル出力（ＴＩＰ＆ＴＡＩＬ用）
        private void outputTbiInfoForCoilAssy()
        {
            ...
            if (sn[0].IndexOf("+") >= 1) sn[0] = VBS.Left(sn[0], 17);

            string newRecord = sn[0] + "," + "null" + "," + model + "," + date + "," + time + "," + lotInf;
            ...
            for (int i = 0; i < jsn.Length; i++)
            {
                ...
                newRecord = sn[0] + "," + "null" + "," + model + "," + date + "," + time + "," + lotInf;
                cumRecords += newRecord + System.Environment.NewLine;
            }
        ...
        private void outputTbiInfoForCrustTungWelding()
        {
            string sn = txtChild.Text.Trim();
            string[] jsn = { txtChild2.Text.Trim() };
            string[] lbl = { lblChild2.Text.Trim() };
            string model = txtModel.Text.Trim();
            string date = DateTime.Today.ToString("yyyy/MM/dd");
            string time = DateTime.Now.ToString("HH:mm:ss");
            string cumRecords = string.Empty;
            ...
            for (int i = 0; i < rowPartsLot; i++)
            {
                string lotInf = (dgvPartsLot[3, i].Value.ToString().Replace(":", "_") + ":" + string.Empty + ":" + description[i] + ":" + dgvPartsLot[0, i].Value.ToString() + ":" +
                    ...
                    .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");
                string newRecord = sn + "," + sn + "," + model + "," + date + "," + time + "," + lotInf;
                cumRecords += newRecord + System.Environment.NewLine;
            }

            for (int i = 0; i < jsn.Length; i++)
            {
                string lotInf = ("-" + ":" + string.Empty + ":" + description[i + 8] + ":" + jsn[i] + ":" +
                    lbl[i].ToUpper() + ":" + "null" + ":" + "2000/01/01")
                    .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");
        ...
        // サブプロシージャ：テキストファイル出力（ＴＩＰ＆ＴＡＩＬ用）
        private void outputTbiInfoForCrustReading()
        {
            string sn = txtChild.Text.Trim();
            string[] jsn = { txtChild2.Text.Trim() };
            string[] lbl = { lblChild2.Text.Trim() };
            string parent = txtParent.Text.Trim();
            string model = txtModel.Text.Trim();
            string date = DateTime.Today.ToString("yyyy/MM/dd");
            string time = DateTime.Now.ToString("HH:mm:ss");
            string cumRecords = string.Empty;
            ...
            for (int i = 0; i < rowPartsLot; i++)
            {
                string lotInf = (dgvPartsLot[3, i].Value.ToString().Replace(":", "_") + ":" + string.Empty + ":" + description[i] + ":" + dgvPartsLot[0, i].Value.ToString() + ":" +
                    dgvPartsLot[1, i].Value.ToString() + ":" + dgvPartsLot[2, i].Value.ToString().Replace(":", "_") + ":" + dgvPartsLot[4, i].Value.ToString())
                    .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");
                string newRecord = sn + "," + "null" + "," + model + "," + date + "," + time + "," + lotInf;
                cumRecords += newRecord + System.Environment.NewLine;
            }

            for (int i = 0; i < jsn.Length; i++)
            {
                string lotInf = (parent + ":" + string.Empty + ":" + description[i + 4] + ":" + jsn[i] + ":" +
                    lbl[i].ToUpper() + ":" + "null" + ":" + "2000/01/01")
                    .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");

                string newRecord = sn + "," + parent + "," + model + "," + date + "," + time + "," + lotInf;
                cumRecords += newRecord + System.Environment.NewLine;
            }
        ...
        private void txtPartsLot_TextChanged(object sender, EventArgs e)
        {

        }

        private void dgvPartsLot_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
