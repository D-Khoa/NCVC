...
namespace JigQuick
{
    public partial class frmDocking : Form
    {
        ...
        // コンストラクタ
        public frmDocking(string applicationname)
        {
        ...
        {
            txtModel.Text = readIni("LABEL DESCRIPTION", "MODEL", appconfig); //Properties.Settings.Default.MODEL_4J;
        ...
        private void txtChild_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (txtChild.Text.Length < 2) return;
        ...
        {
            string sn,sn1,sn2,sn3,sn4;
            string lot,lot1,lot2,lot3,lot4;
            ...
            {
                for (int i = 0; wrongScanOrder == true && i < 6; i++)
                {
                    wrongScanOrder = matchScanOrder(dgvChild[0, i].Value.ToString(), txtChild2.Text, dgvParent[0, i].Value.ToString());
                }
                if (!wrongScanOrder)
                {
            ...
            for (int i = 0; i < rowChildParent; i++)
            {
                sn1 = dgvChild[0, i].Value.ToString();
                if (sn1.IndexOf("+") >= 1) sn1 = VBS.Left(sn1, 17);
                lot1 = "null";
                if (sn1 == string.Empty || lot1 == string.Empty) continue;

                string lotInf = (dgvPartsLot[3, 1].Value.ToString().Replace(":", "_") + ":" + ":" + description[2] + ":" + dgvPartsLot[0, 1].Value.ToString() + ":" +
                    dgvPartsLot[1, 1].Value.ToString() + ":" + dgvPartsLot[2, 1].Value.ToString().Replace(":", "_") + ":" + dgvPartsLot[4, 1].Value.ToString())
                    .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");
                string newRecord = sn1 + "," + lot1 + "," + model + "," + date + "," + time + "," + lotInf;
                cumRecords += newRecord + System.Environment.NewLine;
                tempCount += 1;
            }
            ...
            for (int i = 0; i < rowChildParent; i++)
            {
                sn2 = dgvChild[0, i].Value.ToString();
                if (sn2.IndexOf("+") >= 1) sn2 = VBS.Left(sn2, 17);
                lot2 = "null";
                if (sn2 == string.Empty || lot2 == string.Empty) continue;

                string lotInf = (dgvPartsLot[3, 2].Value.ToString().Replace(":", "_") + ":" + ":" + description[3] + ":" + dgvPartsLot[0, 2].Value.ToString() + ":" +
                    dgvPartsLot[1, 2].Value.ToString() + ":" + dgvPartsLot[2, 2].Value.ToString().Replace(":", "_") + ":" + dgvPartsLot[4, 2].Value.ToString())
                    .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");
                string newRecord = sn2 + "," + lot2 + "," + model + "," + date + "," + time + "," + lotInf;
                cumRecords += newRecord + System.Environment.NewLine;
                tempCount += 1;
            }
            ...
            for (int i = 0; i < rowChildParent; i++)
            {
                sn3 = dgvChild[0, i].Value.ToString();
                if (sn3.IndexOf("+") >= 1) sn3 = VBS.Left(sn3, 17);
                lot3 = txtChild2.Text;
                if (sn3 == string.Empty || lot3 == string.Empty) continue;

                string lotInf = (dgvPartsLot[3, 3].Value.ToString().Replace(":", "_") + ":" + ":" + description[4] + ":" + dgvPartsLot[0, 3].Value.ToString() + ":" +
                    dgvPartsLot[1, 3].Value.ToString() + ":" + dgvPartsLot[2, 3].Value.ToString().Replace(":", "_") + ":" + dgvPartsLot[4, 3].Value.ToString())
                    .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");
                string newRecord = sn3 + "," + lot3 + "," + model + "," + date + "," + time + "," + lotInf;
                cumRecords += newRecord + System.Environment.NewLine;
                tempCount += 1;
            }
            ...
            if (rowPartsLot == 5)
            {
                for (int i = 0; i < rowChildParent; i++)
                {
                    sn4 = dgvChild[0, i].Value.ToString();
                    if (sn4.IndexOf("+") >= 1) sn4 = VBS.Left(sn4, 17);
                    lot4 = txtChild2.Text;
                    if (sn4 == string.Empty || lot4 == string.Empty) continue;

                    string lotInf = (dgvPartsLot[3, 4].Value.ToString().Replace(":", "_") + ":" + ":" + description[5] + ":" + dgvPartsLot[0, 4].Value.ToString() + ":" +
                        dgvPartsLot[1, 4].Value.ToString() + ":" + dgvPartsLot[2, 4].Value.ToString().Replace(":", "_") + ":" + dgvPartsLot[4, 4].Value.ToString())
                        .Replace(" ", "_").Replace(",", "_").Replace("'", "_").Replace(";", "_").Replace("\"", "_");
                    string newRecord = sn4 + "," + lot4 + "," + model + "," + date + "," + time + "," + lotInf;
                    cumRecords += newRecord + System.Environment.NewLine;
                    tempCount += 1;
                }
            }
