...
namespace IpqcDB
{
    public class TfSQL
    {
        ...
        static string conStringIpqcDbP4 = @"Server=192.168.193.4;Port=5432;User Id=ipqc;Password=ipqc;Database=ip_pqmdb; CommandTimeout=100; Timeout=100;";
        ...
        public bool sqlMultipleInsert(string model, string process, string inspect, DateTime lot, DateTime inspectdate, string line, string status, DataTable dt)
        {
            ...
            try
            {
                ...
                // 登録②：
                string sql2 = "insert into tbl_measure_history(model, process, inspect, lot, inspectdate, line, qc_user, row_set, m1, m2, m3, m4, m5, x, r, status) " +
                    "VALUES (:model, :process, :inspect, :lot, :inspectdate, :line, :qc_user, :row_set, :m1, :m2, :m3, :m4, :m5, :x, :r, :status)";
                ...
                command2.Parameters.Add(new NpgsqlParameter("status", NpgsqlTypes.NpgsqlDbType.Text));
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    command2.Parameters[15].Value = status;
