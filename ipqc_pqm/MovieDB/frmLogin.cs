...
namespace IpqcDB
{
    public partial class frmLogin : Form
    {
        ...
        private void Form5_Load(object sender, EventArgs e)
        {
            ...
            string sql = "select DISTINCT qcuser FROM qc_user_temp ORDER BY qcuser";
            TfSQL tf = new TfSQL();
        ...
        private void btnLogIn_Click(object sender, EventArgs e)
        {
            ...
            {
                TfSQL tf = new TfSQL();
                string logt = tf.sqlExecuteScalarString("select login_times from qc_user_temp where qcuser = '" + cmbUserName.Text + "'");
                if (logt == "0")
                {
                ...
                else
                {
                    ...
                    if (user != null)
                    {
                        sql = "select pass FROM qc_user_temp WHERE qcuser='" + user + "'";
                        pass = tf.sqlExecuteScalarString(sql);

                        sql = "select loginstatus FROM qc_user_temp WHERE qcuser='" + user + "'";
                        login = tf.sqlExecuteScalarBool(sql);

                        sql = "select ip_address from qc_user_temp where qcuser = '" + user + "'";
                        ...
                        if (pass == str_md5)
                        {
                            if (login && ip != "null" && ip != lblIP.Text)
                            {
                                ...
                                sql = "UPDATE qc_user_temp SET loginstatus=false, ip_address = '" + lblIP.Text + "' WHERE qcuser='" + user + "'";
                                bool res1 = tf.sqlExecuteNonQuery(sql, false);
                                return;
                            }

                            //Check IP Address
                            if (ip == "null") tf.sqlExecuteScalarString("UPDATE qc_user_temp SET ip_address = '" + lblIP.Text + "' where qcuser = '" + user + "'");

                            // ログイン状態をＴＲＵＥへ変更
                            sql = "UPDATE qc_user_temp SET loginstatus=true WHERE qcuser='" + user + "'";
                            ...
                            sql = "UPDATE qc_user_temp SET loginstatus=false, ip_address = 'null' WHERE qcuser='" + user + "'";
            ...
            else
            {
                ...
                else
                {
                    ...
                    GetMD5(pass);
                    TfSQL up = new TfSQL();
                    up.sqlExecuteScalarString("update qc_user_temp set pass = '" + str_md5 + "' where qcuser = '" + cmbUserName.Text + "'");
                    up.sqlExecuteScalarString("update qc_user_temp set login_times = '1' where qcuser = '" + cmbUserName.Text + "'");
        ...
        private void login()
        {
            ...
            if (user != null)
            {
                ...
                //Check IP Address
                if (ip == "null") tf.sqlExecuteScalarString("UPDATE qc_user_temp SET ip_address = '" + lblIP.Text + "'");
        ...
        private void btnChange_Click(object sender, EventArgs e)
        {
            ...
            TfSQL tf = new TfSQL();
            string passC = tf.sqlExecuteScalarString("select pass from qc_user_temp where qcuser = '" + cmbUserC.Text + "'");
            if (passC == str_md5)
            {
            ...
            tf.sqlExecuteScalarString("update qc_user_temp set pass = '" + str_md5 + "' where qcuser = '" + cmbUserC.Text + "'");
            DialogResult result = MessageBox.Show("Your password has been changed!", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (result == DialogResult.OK) grChangePass.Visible = false;
        }
